    ...
    public static Window[] listOpenedWindows() throws OSNotSupportedException {
        ...
        user32.EnumWindows(new WNDENUMPROC() {
            ...
            public boolean callback(HWND hWnd, Pointer arg1) {
                ...
                int length = 0;
                while (length < windowText.length && windowText[length] != 0) {
                    length++;
                }
                
                String wText;
                try {
                    wText = new String(windowText, 0, length, "Cp1252");
                } catch(UnsupportedEncodingException e) {
//                    wText = Native.toString(windowText);
                    wText = new String(windowText, 0, length);
                }

                // débarrassez-vous de ce bloc si vous voulez que toutes les fenêtres aient ou non du texte
                if (wText.isEmpty() || wText.isBlank())
                    return true;
